    ...
    <div className="bg-white rounded-xl border border-gray-200 p-5 flex flex-col gap-3 hover:shadow-md transition-shadow">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="text-xs text-gray-400 mt-0.5">{repo.owner.login}</p>
        </div>
      ...
      {repo.description && (
        <p className="text-sm text-gray-500 leading-relaxed line-clamp-2">{repo.description}</p>
      )}

      <p className="text-xs text-gray-400">Updated {formatDate(repo.updated_at)}</p>
